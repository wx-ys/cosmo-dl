...
_SIMULATIONS: list[tuple[str, str, str, str, str, int, bool]] = [
    # -- Small test run -------------------------------------------------------
    ("RefL0012N0188", "Fiducial_models", "12.5 Mpc", "188³",
     "Tiny volume reference model", 29, True),

    # -- Schaye et al. (2015) — Fiducial_models -------------------------------
    ("RefL0100N1504", "Fiducial_models", "100 Mpc", "1504³",
     "Large volume reference model", 29, True),
    ("RefL0050N0752", "Fiducial_models", "50 Mpc", "752³",
     "Medium volume reference model", 29, True),
    ("RefL0025N0376", "Fiducial_models", "25 Mpc", "376³",
     "Small volume reference model", 29, True),
    ("RefL0025N0752", "Fiducial_models", "25 Mpc", "752³",
     "High resolution reference model", 29, True),
    ("RecalL0025N0752", "Fiducial_models", "25 Mpc", "752³",
     "High resolution recalibrated model", 29, True),
    ("AGNdT9L0050N0752", "Fiducial_models", "50 Mpc", "752³",
     "Medium volume with adjusted AGN heating", 29, True),

    # -- Crain et al. (2015) — Physics_vars -----------------------------------
    ("FBconstL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "FBconst in Crain+2015 table 1", 29, True),
    ("FBsigmaL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "FBσ in Crain+2015 table 1", 29, True),
    ("FBZL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "FBZ in Crain+2015 table 1", 29, True),
    ("eos1L0025N0376", "Physics_vars", "25 Mpc", "376³",
     "eos1 in Crain+2015 table 1", 29, True),
    ("eos53L0025N0376", "Physics_vars", "25 Mpc", "376³",
     "eos5/3 in Crain+2015 table 1", 29, True),
    ("FixedSfThreshL0025N0376", "Physics_vars", "25 Mpc", "376³",
     "FixedSfThresh in Crain+2015 table 1", 29, True),
    ("WeakFBL0025N0376", "Physics_vars", "25 Mpc", "376³",
     "WeakFB in Crain+2015 table 1", 29, True),
    ("StrongFBL0025N0376", "Physics_vars", "25 Mpc", "376³",
     "StrongFB in Crain+2015 table 1", 29, True),
    ("ViscLoL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "ViscLo in Crain+2015 table 1", 29, True),
    ("ViscHiL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "ViscHi in Crain+2015 table 1", 29, True),
    ("C15AGNdT8L0050N0752", "Physics_vars", "50 Mpc", "752³",
     "AGNdT8 in Crain+2015 table 1", 29, True),
    ("C15AGNdT9L0050N0752", "Physics_vars", "50 Mpc", "752³",
     "AGNdT9 in Crain+2015 table 1", 29, True),

    # -- Barber et al. (2018) — Physics_vars ----------------------------------
    ("HiML0050N0752", "Physics_vars", "50 Mpc", "752³",
     "IMF top-heavy at high birth pressure — 11 snapshots", 11, True),
    ("LoML0050N0752", "Physics_vars", "50 Mpc", "752³",
     "IMF bottom-heavy at high birth pressure — 11 snapshots", 11, True),

    # -- No black holes — Physics_vars ----------------------------------------
    ("NoAGNL0025N0376", "Physics_vars", "25 Mpc", "376³",
     "Small volume with no black holes", 29, True),
    ("NoAGNL0050N0752", "Physics_vars", "50 Mpc", "752³",
     "Medium volume with no black holes", 29, True),

    # -- Dark matter only — DMONLY --------------------------------------------
    ("L0025N0376", "DMONLY", "25 Mpc", "376³",
     "Small volume DMONLY model", 0, False),
    ("L0025N0752", "DMONLY", "25 Mpc", "752³",
     "Small volume DMONLY at high resolution", 0, False),
    ("L0100N1504", "DMONLY", "100 Mpc", "1504³",
     "Large volume DMONLY model", 0, False),
]
...
def _build_snapshot_children(
    ...
    for snap_num in range(num_snapshots):
        ...
        download_url = (
            f"{EAGLE_BASE_URL}/download"
            f"?run={sim_name}&snapnum={snap_num}"
        )
...
def _build_schema_children(schema: str) -> dict[str, SourceNode]:
    ...
    for run, s, box, particles, notes, n_snap, has_snaps in _SIMULATIONS:
        ...
        children[run] = SourceNode(
            ...
            _loader=lambda r=run, sp=sim_path, ns=n_snap: _build_simulation_children(r, sp, ns),
        )
    ...
    def _load_root_children() -> dict[str, SourceNode]:
        ...
        for schema, desc in _SCHEMA_ORDER:
            ...
            children[schema] = SourceNode(
                ...
                _loader=lambda sc=schema: _build_schema_children(sc),
            )
        return children
